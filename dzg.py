...
class Trade:
    ...
    def EditOrder(self, order_id, symbol, side, type_, quantity, price=None, stop_loss=None, take_profit=None):
    ...
    def ListOfTrades(self, start_time: None| int = None, end_time: None| int = None, symbol: None| str = None, limit: None| int = None):
    ...
    def ListOfLeverageTrades(self):
        ...
        query_string = f"timestamp={timestamp}&recvWindow={self.recv_window}"
        signature = self._generate_signature(query_string)
        url = f"{self.url}/tradingPositions?{query_string}&signature={signature}"
    ...
    def CencelOrder(self, order_id:str, symbol:str):
...
# result = trade.AccountInfo()
result = trade.ListOfTrades(symbol="Gold")



# # Размещение ордера
# result = trade.CreateOrder(
#     symbol="Gold",
#     side="BUY",
#     type_="LIMIT",
#     quantity=0.1,
...
# # Изменение ордера
# result = trade.LeverageOrdersEdit(
...
# Отмена ордера
# result = trade.CencelOrder(
#     order_id="28601567-1e55-311e-0000-00008087365c", 
#     symbol="Gold"
#     )


# Изменение торгуемой позиции
# edit = trade.LeverageTradeEdit(
        # position_id='2854ed76-1e15-549e-0000-0000808b0005', #параметр id 
#         take_profit=38.28
# )

##Закрытие торгуемой позиции
# result = trade.TradingPositionClose(
